...
def check_shape(
    ...
    for i, layer in enumerate(model.named_modules()):
        ...
        if layer_type == nn.Linear:
            if (
                not _is_available_shape(
                    shape_history[i - 1]["out"],
                    _layer.in_features,
                    shape_history[i - 1]["shape"],
                    "1d",
                )
                and not is_no_shape_check
            ):
                raise InvalidShapeError(f"Specified model is invalid.")

            shape_history[i] = {
                "in": _layer.in_features,
                "out": _layer.out_features,
                "layer": "Linear",
                "shape": "1d",
            }
            ...
            shape_history[i] = {
                "in": shape_history[i - 1]["out"],
                "out": shape_history[i - 1]["out"],
                "shape": shape_history[i - 1]["shape"],
            ...
            _output_shape = _calculate_convolutional_output_shape(
                shape_history[i - 1]["out"],
                _layer.out_channels,
                _layer.kernel_size,
                _layer.padding,
            ...
            _output_shape = _calculate_pooling_output_shape(
                shape_history[i - 1]["out"],
                _layer.kernel_size,
                _layer.padding,
                _layer.stride,
            ...
            _output_shape = _calculate_upsample_shape(
                shape_history[i - 1]["out"], _layer.scale_factor
            )
            shape_history[i] = {
                "in": shape_history[i - 1]["out"],
                "out": _output_shape,
            ...
            shape_history[i] = {
                "in": shape_history[i - 1]["out"],
                "out": shape_history[i - 1]["out"],
                "layer": f"Batch Normalization {_shape}",
                "shape": _shape,
            }
...
def _is_available_shape(
    ...
    previous_output_shape: List[int],
    current_input_shape: List[int],
) -> bool:
    """_is_available_shape
...
def _calculate_convolutional_output_shape(
    input_shape, output_chennels, kernel_size, padding=(0, 0), stride=(1, 1)
) -> Tuple[int, int, int]:
    ...
    """
    output_height = ceil(
        (input_shape[0] + 2 * padding[0] - kernel_size[0]) / stride[0] + 1
    )
    output_width = ceil(
        (input_shape[1] + 2 * padding[1] - kernel_size[1]) / stride[1] + 1
    )
...
def _calculate_pooling_output_shape(
    ...
    output_height = floor(
        (input_shape[0] + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1)
        / stride[0]
        + 1
    )
    output_width = floor(
        (input_shape[1] + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1)
        / stride[1]
        + 1
    )
