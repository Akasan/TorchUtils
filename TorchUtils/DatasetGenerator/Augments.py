...
class DataAugmentationGenerator:
    ...
    def _append(self, transform, is_random_apply=False, random_apply_rate=0.5):
        if is_random_apply:
            self.transform.transforms.append(
                transforms.RandomApply([transform], p=random_apply_rate)
            )
        else:
            self.transform.transforms.append(transform)

    def add_resize(
        self, new_size, interpolation=2, is_random_apply=False, random_apply_rate=0.5
    ):
        """add_resize
        ...
        """
        self._append(
            transforms.Resize(new_size, interpolation),
            is_random_apply,
            random_apply_rate,
        )
    ...
    def add_center_crop(self, crop_size, is_random_apply=False, random_apply_rate=0.5):
        ...
        """
        self._append(
            transforms.CenterCrop(crop_size), is_random_apply, random_apply_rate
        )
    ...
    def add_pad(
        ...
        self._append(
            transforms.Pad(padding, fill=fill, padding_mode=padding_mode),
            is_random_apply,
            random_apply_rate,
        )

    def add_random_affine(
        self,
        degree,
        translate=None,
        scale=None,
        ...
        self._append(
            transforms.RandomAffine(degree, translate, scale, shear, resample),
            is_random_apply,
            random_apply_rate,
        )
    ...
    def add_random_crop(self, crop_size, is_random_apply=False, random_apply_rate=0.5):
        ...
        """
        self._append(
            transforms.RandomCrop(crop_size), is_random_apply, random_apply_rate
        )

    def add_random_horizontal_flip(
        self, p=0.5, is_random_apply=False, random_apply_rate=0.5
    ):
        ...
        """
        self._append(
            transforms.RandomHorizontalFlip(p), is_random_apply, random_apply_rate
        )

    def add_random_vertical_flip(
        self, p=0.5, is_random_apply=False, random_apply_rate=0.5
    ):
        ...
        """
        self._append(
            transforms.RandomVerticalFlip(p), is_random_apply, random_apply_rate
        )
    ...
    def add_normalize(self, mean, std, is_random_apply=False, random_apply_rate=0.5):
        ...
        self._append(
            transforms.Normalize(mean, std), is_random_apply, random_apply_rate
        )
